...
# 1. Read the JSON from the FILE PATH passed by C#
payload_file_path = sys.argv[1]
with open(payload_file_path, "r", encoding="utf-8") as f:
    upload_material = json.load(f)

# 2. Handle Patient Info safely
if len(sys.argv) > 2:
    patient_info = json.loads(sys.argv[2])
else:
    # Mock patient data since C# isn't passing real patient info yet
    patient_info = {"ID": "MOCK-123", "Name": "Jane Doe"}
...
def mock_push_to_fhir_server(payload):
    # Added indent=2 to make the output look clean and readable in your C# chat box
